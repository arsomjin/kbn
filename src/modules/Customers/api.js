...
export const createCustomerNo = (branchCode, TS, suffix = 'KBN-CUS') => {
  ...
  const orderId = `${suffix}-${branchCode || ''}${dayjs(TS).format('YYYYMMDD')}${padLastNo}`;
  return orderId;
};
...
export const getInitValues = (selectedCustomer, user, branchCode) => {
  ...
  return {
    ...
    whenToBuyRange: selectedCustomer?.whenToBuyRange || {
      range: [dayjs().format('YYYY-MM-DD'), dayjs().add(1, 'month').format('YYYY-MM-DD')],
      months: [dayjs().format('YYYY-MM'), dayjs().add(1, 'month').format('YYYY-MM')],
    },
  };
};
