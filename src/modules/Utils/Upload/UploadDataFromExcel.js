...
import { showLog, formatExcelToJson, showWarn, showAlert, showSuccess, errorHandler } from 'functions';
...
export default () => {
  ...
  const _onDataLoaded = async (dataLoaded, byPassDuplicateCheck) => {
    try {
      ...
      const isDataCorrected = await _checkImportData(nData, jsonData, dataType, firestore, users, byPassDuplicateCheck);
      //  showLog({ isDataCorrected });
      if (!isDataCorrected.result) {
        //
        showAlert(`ไม่สำเร็จ`, isDataCorrected.info, 'warning', handleCancel);
        // Allow re-upload duplicate data. (เขียนทับของเดิม)
        // showAlert(
        //   `ไม่สำเร็จ ${isDataCorrected.info}`,
        //   'ต้องการอัปโหลดข้อมูลอีกครั้งหรือไม่ ?',
        //   'warning',
        //   () => _checkImportData(dataLoaded, true),
        //   handleCancel
        // );
        return;
      }
  ...
  const _onConfirm = useCallback(async () => {
    try {
      ...
      if (batchNo) {
        ...
        showSuccess(() => handleCancel(), `นำเข้าข้อมูล ${getTitle(dataType)} เรียบร้อยแล้ว`, true);
      }
    } catch (e) {
      showWarn(e);
      errorHandler({
  ...
  return (
    <Container fluid className="main-content-container p-4 bg-light">
      <Excels
      ...
      <div className="border-bottom bg-white p-3 text-right">
        ...
        <Popconfirm
          ...
          cancelText="ยกเลิก"
        >
          <Button type="primary" disabled={!cState.loaded} icon={<UploadOutlined />} className="mr-2 my-2">
            อัปโหลดข้อมูล
          </Button>
        </Popconfirm>
      </div>
