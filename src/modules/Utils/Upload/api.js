...
import { checkCollection } from 'services/firebase';
    ...
    try {
      ...
      if (!['partList', 'vehicleList', 'fingerPrint'].includes(dataType) && !byPassDuplicateCheck) {
        ...
        if (!isNotDuplicated.result) {
          r({
            result: false,
            info: `ไฟล์นี้ได้อัปโหลดแล้ว เมื่อวันที่ ${moment(
              isNotDuplicated.duplicateData.importTime,
            ).format('DD/MM/YY')} เวลา ${moment(isNotDuplicated.duplicateData.importTime).format(
              'HH:mm',
  ...
  new Promise(async (r, j) => {
    try {
      ...
      await arrayForEach(distinctOrders, async (order) => {
        ...
        const priceBeforeDiscount = sItems.reduce(
          (sum, elem) => sum + Numb(elem.priceBeforeDiscount),
          0,
        );
        const SKCDiscountPrice = sItems.reduce((sum, elem) => sum + Numb(elem.SKCDiscountPrice), 0);
        const dealerDiscountPrice = sItems.reduce(
          (sum, elem) => sum + Numb(elem.dealerDiscountPrice),
          0,
        );
        ...
        const wageItems = sItems.filter(
          (l) => l.categoryDesc === 'ค่าแรง' || l.description.includes('ค่าแรง'),
        );
        ...
        const partBeforeDiscount = partItems.reduce(
          (sum, elem) => sum + Numb(elem.priceBeforeDiscount),
          0,
        );
        const partSKCDiscount = partItems.reduce(
          (sum, elem) => sum + Numb(elem.SKCDiscountPrice),
          0,
        );
        const partDealerDiscount = partItems.reduce(
          (sum, elem) => sum + Numb(elem.dealerDiscountPrice),
          0,
        );
        const parts = partItems.reduce((sum, elem) => sum + Numb(elem.netPrice), 0);
        const oilBeforeDiscount = oilItems.reduce(
          (sum, elem) => sum + Numb(elem.priceBeforeDiscount),
          0,
        );
        const oilSKCDiscount = oilItems.reduce((sum, elem) => sum + Numb(elem.SKCDiscountPrice), 0);
        const oilDealerDiscount = oilItems.reduce(
          (sum, elem) => sum + Numb(elem.dealerDiscountPrice),
          0,
        );
        ...
        let percentDiscount =
          partBeforeDiscount + oilBeforeDiscount > 0
            ? parseInt(
                ((partSKCDiscount + oilSKCDiscount) / (partBeforeDiscount + oilBeforeDiscount)) *
                  100,
              )
            : 0;
    ...
    try {
      ...
      await arrayForEach(distinctOrders, async (order) => {
        ...
        let sItems = dataArr
          ...
          .map((sItem) => {
            const { productTypeDesc, netTotal } = sItem;
            return {
              ...sItem,
    ...
    try {
      ...
      switch (dataType) {
        ...
        case 'services':
          ...
          let serviceItemsRef = firestore
            .collection('sections')
            .doc('services')
            .collection('importServiceItems');
        ...
        case 'partList':
          ...
          break;
        case 'fingerPrint':
          let fingerPrintRef = firestore
            .collection('sections')
            .doc('hr')
            .collection('importFingerPrint');
          let fingerPrintBatchRef = firestore
            .collection('sections')
            .doc('hr')
            .collection('importFingerPrintBatch');
          let plBreak11 = false;
          ...
          await arrayForEach(currentData, async (item, i) => {
            ...
            if (item.employeeCode) {
              ...
              await fingerPrintRef
                .doc(`${item.employeeCode.toString()}-${item.date}`)
                .set(saveItem);
              // Data will be recorded for each batch.
              await fingerPrintBatchRef
                .doc(`${item.employeeCode.toString()}-${item.date}-${batchNo}`)
                .set(saveItem);
            }
          });
          if (plBreak11) {
