import { checkCollection, checkDoc } from 'services/firebase';
...
const getMonths = ([startDate, stopDate]) => {
  ...
  while (dayjs(currentDate).isSameOrBefore(dayjs(stopDate))) {
    monthArray.push(dayjs(currentDate, 'YYYY-MM-DD').format('YYYY-MM'));
    currentDate = dayjs(currentDate, 'YYYY-MM-DD').add(1, 'month').format('YYYY-MM-DD');
  }
  return monthArray;
};
...
export const getWhenToBuyRange = (wtb, TS) => {
  ...
  let taday = TS ? dayjs(TS).format('YYYY-MM-DD') : dayjs().format('YYYY-MM-DD');
  if (!wtb) {
    return null;
  }
  let range = [taday, dayjs().add(1, 'month').format('YYYY-MM-DD')];
  switch (wtb) {
    case 'สัปดาห์นี้':
    case 'thisWeek':
      range[1] = dayjs().add(1, 'week').format('YYYY-MM-DD');
      break;
    case 'เดือนนี้':
      range[1] = dayjs().add(1, 'month').format('YYYY-MM-DD');
      break;
    case 'ภายใน 3 เดือน':
    case 'threeMonth':
      range[1] = dayjs().add(3, 'month').format('YYYY-MM-DD');
      break;
    case 'ปีนี้':
    case 'thisYear':
      range[1] = dayjs().endOf('year').format('YYYY-MM-DD');
      break;
    case 'ปีหน้า':
    case 'nextYear': {
      let nextYearFromNow = dayjs().add(1, 'year');
      range[1] = dayjs(nextYearFromNow).endOf('year').format('YYYY-MM-DD');
      break;
    }
    case 'อีกนาน':
    case 'longTime':
      // Long time estimated 10 years.
      range[1] = dayjs().add(10, 'year').format('YYYY-MM-DD');
      break;
    default:
      break;
  }
...
export const formatVehicleItemData = (arr, dataIndex, rowIndex) =>
  new Promise(async (resolve, reject) => {
    try {
        ...
        const newData = await Promise.all(
          arr.map(async (item) => {
            ...
            if (Array.isArray(mItem.vehicleNo) && mItem.vehicleNo.length > 0) {
              const sSnap = await checkCollection('sections/stocks/vehicles', [
                ['vehicleNo', 'in', mItem.vehicleNo],
              ]);
              if (sSnap) {
                let peripheralNo = '';
                let engineNo = '';
      ...
      if (
        dataIndex === 'vehicleNo' &&
        Array.isArray(mItem.vehicleNo) &&
        mItem.vehicleNo.length > 0
      ) {
        const sSnap = await checkCollection('sections/stocks/vehicles', [
          ['vehicleNo', 'in', mItem.vehicleNo],
        ]);
    ...
    try {
      ...
      if (dataIndex === 'pCode' && arr[rowIndex][dataIndex]) {
        ...
        if (sDoc) {
          ...
          mItem.total = (
            Numb(sDoc.data().slp) * Numb(mItem.qty) -
            Numb(sDoc.data()?.discount || 0)
          ).toFixed(2);
          ...
          arr.map(async (item) => {
            ...
            if (mItem.serviceCode) {
              const sDoc = await checkDoc(
                'data',
                isPart
                  ? `products/partList/${mItem.serviceCode}`
                  : `services/serviceList/${mItem.serviceCode}`,
              );
              if (sDoc) {
...
export const getExtraSaleSnap = (items) => {
  ...
  let allItems = (items || []).map((it) => {
    ...
    let isVehicle = it?.productName
      ? checkIsVehicleFromName(it.productName)
      : vehicleArr.length > 0;
    let modelName =
