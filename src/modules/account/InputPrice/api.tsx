            ...
            <Descriptions column={1} size='small' bordered={false} layout='vertical'>
              ...
              {record?.peripheralNo && (
                <Descriptions.Item label={t('peripheralNumber')}>
                  {Array.isArray(record.peripheralNo) ? record.peripheralNo.join(', ') : record.peripheralNo}
                </Descriptions.Item>
              )}
            </Descriptions>
          </Col>
...
export const getWarehouseCheckedData = async (firestore: Firestore): Promise<InputPriceItem[]> => {
  try {
    ...
    const q = query(stockImportRef, where('warehouseChecked', '!=', null), where('total', '==', null));
    ...
    return sortArr(stockImportArr as unknown as Record<string, unknown>[], 'importTime').map((od, id) => ({
      ...od,
      id,
      key: id.toString(),
