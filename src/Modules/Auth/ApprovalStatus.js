...
const ApprovalStatus = ({ userData, user, onBackToLogin }) => {
  ...
  const isRejected =
    currentUser?.approvalStatus === 'rejected' && currentUser?.rejectedAt; // Must have been explicitly rejected (no time delay needed)

  // Check if user is a resigned employee pending approval
  const isResignedEmployee =
    currentUser?.isResignedEmployee === true ||
  ...
  if (process.env.NODE_ENV === 'development' && currentUser) {
    const rejectionDebug = {
      ...
      timeSinceRejection: currentUser?.rejectedAt
        ? Date.now() - currentUser.rejectedAt
        : 0,
    ...
    if (currentUser?.approvalStatus === 'rejected' && !isRejected) {
      console.log(
        '⚠️ User has rejected status but not showing rejection screen:',
        {
          reason: !currentUser?.rejectedAt
            ? 'Missing rejectedAt timestamp'
  ...
  const approvalLevel =
    currentUser?.approvalLevel ||
    (isResignedEmployee
      ? 'province_manager'
      : currentUser?.access?.authority === 'ADMIN'
        ? 'super_admin'
        : currentUser?.access?.authority === 'MANAGER'
          ? 'province_manager'
          : currentUser?.access?.authority === 'LEAD'
            ? 'branch_manager'
  ...
  useEffect(() => {
    if (process.env.NODE_ENV === 'development' && currentUser) {
      ...
      const hasLegacyAuth = !!(
        currentUser.accessLevel || currentUser.homeProvince
      );
      ...
      if (!province || province === 'นครสวรรค์') {
        console.warn(
          '⚠️ ApprovalStatus: Using default province (Nakhon Sawan)'
        );
      }
  ...
  useEffect(() => {
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      window.rejectCurrentUser = async (
        reason = 'ข้อมูลไม่ครบถ้วนสำหรับการทดสอบ'
      ) => {
      ...
      console.log(
        '  - window.rejectCurrentUser("reason") - Reject current user in Firebase'
      );
    }
  ...
  if (isRejected) {
    ...
    const rejectedBy =
      currentUser?.rejectedBy || currentUser?.rejectorName || 'ผู้ดูแลระบบ';
    const rejectedAt = currentUser?.rejectedAt
      ? new Date(currentUser.rejectedAt).toLocaleDateString('th-TH')
      : 'ไม่ทราบวันที่';
          ...
          >
            ...
            <div
              style={{
                ...
                bottom: 0,
                background:
                  'linear-gradient(135deg, rgba(45, 80, 22, 0.15) 0%, rgba(26, 78, 44, 0.25) 50%, rgba(45, 80, 22, 0.15) 100%)',
            ...
            <div
              className='glass-card glass-error'
              style={{
                maxWidth: '600px',
                width: '100%',
                padding: '48px',
            ...
            >
              <div style={{ marginBottom: '32px' }}>
                ...
                <Title
                  level={2}
                  style={{ color: '#cf1322', marginBottom: '16px' }}
                >
                  คำขอถูกปฏิเสธ
                </Title>
              ...
              >
                ...
                >
                  <Text strong style={{ color: '#2d5016' }}>
                    เหตุผลที่ปฏิเสธ:
                  </Text>
                  <br />
              ...
              {/* Contact Information */}
              <div
                className='glass-card glass-info'
              ...
              <Row
                gutter={[20, 16]}
                justify='center'
                className='reapplication-buttons-row'
              >
        ...
        >
          <ReapplicationForm
            ...
            onSuccess={(data) => {
              ...
              console.log(
                '🎉 Reapplication submitted successfully, triggering auth refresh...'
              );
          ...
          <div className='nature-login-container'>
            <div className='nature-auth-card'>
              ...
              >
                <Title
                  level={4}
                  style={{ margin: '0 0 16px 0', color: '#2d5016' }}
                >
                  ข้อมูลการสมัคร
                </Title>
                <Space
              ...
              >
                <Title
                  level={4}
                  style={{ margin: '0 0 24px 0', color: '#2d5016' }}
                >
                  สถานะการดำเนินการ
                </Title>
                <Steps
              ...
              >
                ...
                >
                  {(() => {
                    const phoneNumbers = getAdditionalPhoneNumbers(
