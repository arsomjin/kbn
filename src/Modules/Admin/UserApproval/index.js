...
import { ApprovalPopconfirm, RejectPopconfirm } from '../../../components/StandardPopconfirm';
...
const UserApproval = () => {
  ...
  const handleApprove = async (requestId, requestData) => {
    ...
    try {
      ...
      const userId = userData.userId || userData.uid || requestData.userId || requestData.uid;
      ...
      message.success('อนุมัติผู้ใช้เรียบร้อยแล้ว - ผู้ใช้สามารถเข้าใช้งานได้ทันที');
      fetchApprovalRequests();
      
    } catch (error) {
      console.error('❌ Error approving user:', error);
      console.error('❌ Request data causing error:', requestData);
      message.error('ไม่สามารถอนุมัติผู้ใช้ได้: ' + (error.message || 'เกิดข้อผิดพลาดไม่ทราบสาเหตุ'));
    }
  ...
  const handleReject = async (requestId, reason) => {
    setActionLoading(true);
    try {
      console.log('🔄 Rejecting approval request:', requestId, 'Reason:', reason);
      
      // Get the approval request data
      const requestDoc = await app.firestore()
        ...
        <Space size="middle">
          ...
          {record.status === 'pending' && hasPermission('users.approve', {
            ...
            <>
              ...
              <RejectPopconfirm
                onConfirm={() => showRejectModal(record.id)}
              >
                <Button
                  danger
                  icon={<CloseOutlined />}
  ...
  const handleRejectConfirm = async () => {
    ...
    const selectedReasonObj = REJECTION_REASONS.find(r => r.value === rejectionReason);
            ...
            <>
              <Descriptions column={1} bordered>
                ...
                <Descriptions.Item label="จังหวัด">
                  {getProvinceName(selectedRequest.targetProvince) || selectedRequest.targetProvince || 'ไม่ระบุ'}
                </Descriptions.Item>
                <Descriptions.Item label="สาขา">
                  {getBranchName(selectedRequest.targetBranch) || selectedRequest.targetBranch || 'ไม่ระบุ'}
                </Descriptions.Item>
                ...
                <div style={{ marginTop: '24px' }}>
                  ...
                  <Descriptions bordered size="small" column={1}>
                    <Descriptions.Item label="เหตุผลที่ถูกปฏิเสธ">
                      <Text type="danger">
                        {selectedRequest.previousRejection?.reason || 'ไม่ได้ระบุเหตุผล'}
                      </Text>
                    </Descriptions.Item>
                    <Descriptions.Item label="วันที่ถูกปฏิเสธ">
                      {selectedRequest.previousRejection?.rejectedAt ? 
                        new Date(selectedRequest.previousRejection.rejectedAt).toLocaleString('th-TH') : 
                        'ไม่ทราบวันที่'
                      }
                    </Descriptions.Item>
                    <Descriptions.Item label="ปฏิเสธโดย">
                      {selectedRequest.previousRejection?.rejectedBy || 'ไม่ทราบ'}
                    </Descriptions.Item>
                  </Descriptions>
