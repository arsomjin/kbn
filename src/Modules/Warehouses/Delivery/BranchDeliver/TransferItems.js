...
import { formatItemData } from 'Modules/Warehouses/Vehicles/api';

const TransferItems = ({ deliverId, items, onChange, grant, readOnly }) => {
  ...
  const onAddNewItem = async dArr => {
    try {
      let newItem = {
        ...
        id: dArr.length,
        key: dArr.length,
        deliverItemId: getItemId('PLAN-BRANCH'),
        deliverId,
  ...
  return (
    <EditableRowTable
      ...
      miniAddButton
      summary={pageData => (
        <TableSummary pageData={pageData} dataLength={data.length} startAt={grant ? 4 : 3} sumKeys={['qty']} />
      )}
      pagination={false}
      size="small"
      disabled={!grant}
