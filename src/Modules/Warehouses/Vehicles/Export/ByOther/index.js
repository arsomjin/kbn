...
export default () => {
  ...
  useEffect(() => {
    ...
    let isEdit = !!pOrder && !!pOrder.exportId && !!pOrder.created;
    ...
    if (!isEdit) {
      let exportId = createNewOrderId('WH-VEH');
      pOrder = { exportId };
      setProps({
        order: pOrder,
        isEdit,
    ...
    setBranch(pOrder?.fromOrigin || user.branch || '0450');
  ...
  const _onValuesChange = val => {
    ...
    if (changeKey === 'destination') {
      let mItems = form.getFieldValue('items');
      form.setFieldsValue({
        items: mItems.map(l => ({
          ...l,
          destination: val[changeKey]
        }))
      });
    }
    if (changeKey === 'isUsed') {
  ...
  const _resetToInitial = async () => {
    let exportId = createNewOrderId('WH-VEH');
    form.resetFields();
    setProps({ ...initProps, order: { exportId } });
    await waitFor(0);
    form.setFieldsValue({
      ...getInitialValues(user, { exportId }),
      branchCode: branch,
      fromOrigin: branch,
      exportDate: date
    });
  ...
  const _preConfirm = async values => {
    try {
      ...
      const dupSnap = await checkCollection('sections/stocks/otherVehicleOut', [
  ...
  const _onConfirm = async (mValues, mItems) => {
    try {
      ...
      const transferRef = firestore.collection('sections').doc('stocks').collection('otherVehicleOut');
      await transferRef.doc(mValues.exportId).set({
        ...sValues,
  ...
  const onUpdate = async row => {
    try {
      ...
      delete nValues.id;
      // Check editing.
      let newValues = checkEditRecord(nValues, data, user, nValues.exportId);
      await api.updateItem(newValues, 'sections/stocks/otherVehicleOut', newValues.exportId);
      load(false);
    } catch (e) {
      showWarn(e);
      load(false);
  ...
  const onDelete = async key => {
    try {
      ...
      delete nValues.id;
      let newValues = checkEditRecord(nValues, data, user, nData[index].exportId);
      await api.updateItem(newValues, 'sections/stocks/otherVehicleOut', nData[index].exportId);
      load(false);
    } catch (e) {
      showWarn(e);
      load(false);
