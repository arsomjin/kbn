...
export default () => {
  ...
  const _onConfirm = async values => {
    try {
      ...
      let _key = createNewId('WH-IM-OTH');
      await api.setItem(
        {
          ...sValues,
          deleted: false,
        ...
        'sections/stocks/otherVehicleIn',
        _key
      );
  ...
  const onUpdate = async row => {
    try {
      ...
      await api.updateItem(newValues, 'sections/stocks/otherVehicleIn', newValues._key);
      load(false);
    } catch (e) {
      showWarn(e);
      load(false);
  ...
  const onDelete = async key => {
    try {
      ...
      delete nValues.id;
      let newValues = checkEditRecord(nValues, data, user);
      await api.updateItem(newValues, 'sections/stocks/otherVehicleIn', nData[index]._key);
      load(false);
    } catch (e) {
      showWarn(e);
      load(false);
