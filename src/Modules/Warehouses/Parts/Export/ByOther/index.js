...
export default () => {
  ...
  const _onConfirm = async values => {
    try {
      ...
      let _key = createNewId('WH-EX-OTH');
      await api.setItem(
        {
          ...sValues,
          deleted: false,
        ...
        'sections/stocks/otherVehicleOut',
        _key
      );
      //  showLog({ addTransferOut: addTransferOut.id });
      load(false);
      showSuccess(() => _resetToInitial(), `บันทึกสินค้าออก ${sValues.docNo} สำเร็จ`, true);
    } catch (e) {
      showWarn(e);
      load(false);
      errorHandler({
  ...
  const onUpdate = async row => {
    try {
      ...
      await api.updateItem(newValues, 'sections/stocks/otherVehicleOut', newValues._key);
      load(false);
    } catch (e) {
      showWarn(e);
      load(false);
  ...
  const onDelete = async key => {
    try {
      ...
      delete nValues.id;
      let newValues = checkEditRecord(nValues, data, user);
      await api.updateItem(newValues, 'sections/stocks/otherVehicleOut', nData[index]._key);
      load(false);
    } catch (e) {
      showWarn(e);
      load(false);
