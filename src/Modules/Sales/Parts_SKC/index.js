...
import { showLog, showWarn, distinctArr, toCurrency } from 'functions';
...
export default ({}) => {
  ...
  const _getData = useCallback(async sValues => {
    try {
      showLog({ sValues });
      if (sValues.branchCode === null) {
        // if branchCode is null, getSearchData will return no data.
        sValues.branchCode = undefined;
      }
      setLoading(true);
      let sData = [];
      ...
      let dData = distinctArr(
        mData,
        ['saleDate', 'branchCode', 'sourceOfData'],
        ['netTotal', 'amtOilType', 'amtPartType']
      );
      ...
      discountArr = discountArr.map((it, id) => ({
        ...
        skcpartsdiscountamount20: toCurrency(it.skcpartsdiscountamount20),
            ...
            <>
              ...
              <Tabs type="card">
                <TabPane tab="รายได้" key="1">
                  <div
                    style={{
                      width: TAB_WIDTH + (isMobile ? 0 : 100),
                      height: TAB_HEIGHT
                    }}
                  >
                    <EditableCellTable
