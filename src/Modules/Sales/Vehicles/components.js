    ...
    >
      <div className=" bg-white">
        ...
        <div className="border bg-light px-3 pt-3 my-3">
          ...
          {/* <div>
            {`วิธีชำระเงิน 1:  ${
              depositPaymentType1 && PaymentType[depositPaymentType1]
                ? PaymentType[depositPaymentType1]
                : depositPaymentType1 || '-'
            }`}
          </div>
          <Row form className="my-3">
            <Col md="6">
              <Input
                ...
                addonAfter="บาท"
                readOnly
                value={numeral(depositPayAmount1).format('0,0.00')}
              />
            </Col>
            {values.depositPaymentType1 === 'transfer' && (
              <Col md="6">
                <Input
                  addonBefore="ธนาคาร"
                  addonAfter="บาท"
                  readOnly
                  value={
                    depositBankAcc1 && banks[depositBankAcc1]
                      ? banks[depositBankAcc1]
                      : depositBankAcc1 || '-'
                  }
                />
              </Col>
          ...
          <div>
            {`วิธีชำระเงิน 2:  ${
              depositPaymentType2 && PaymentType[depositPaymentType2]
                ? PaymentType[depositPaymentType2]
                : depositPaymentType2 || '-'
            }`}
          </div>
          <Row form className="mt-3">
            <Col md="6">
              <Input
                ...
                addonAfter="บาท"
                readOnly
                value={numeral(depositPayAmount2).format('0,0.00')}
              />
            </Col>
            {values.depositPaymentType2 === 'transfer' && (
              <Col md="6">
                <Form.Item label="ธนาคาร">
                  <Input
                    ...
                    readOnly
                    value={
                      depositBankAcc2 && banks[depositBankAcc2]
                        ? banks[depositBankAcc2]
                        : depositBankAcc2 || '-'
                    }
                  />
                </Form.Item>
          ...
          {(values.depositPaymentType1 === 'transfer' ||
            values.depositPaymentType2 === 'transfer') &&
            reservationDepositor && (
              <Row form className="mt-3">
                <Col md="4">
                  <Form.Item label="ชื่อผู้โอน/ฝากเงิน">
                    <div>
                      {reservationDepositor && employees[reservationDepositor]
                        ? `${employees[reservationDepositor].firstName}${
                            employees[reservationDepositor].nickName
                              ? `(${employees[reservationDepositor].nickName})`
                              : ''
                          }`
                        : reservationDepositor || '-'}
