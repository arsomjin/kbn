...
import { showWarn, showActionSheet, showSuccess, getChanges, load } from 'functions';
...
const CustomerDetails = () => {
  ...
  const _onConfirm = async mValues => {
    ...
    try {
      ...
      load(false);
      showSuccess(() => history.goBack(), 'บันทึกข้อมูลสำเร็จ');
      showSuccess(
        () =>
          onBack?.path
            ...
            <Card small className="mb-4">
              <CardHeader className="border-bottom">
                <Row form style={{ justifyContent: 'space-between' }}>
                  ...
                  <h6 className="m-0 mr-3 text-primary">{!values.isNewCustomer ? 'ลูกค้าเก่า' : 'ลูกค้าใหม่'}</h6>
                </Row>
              </CardHeader>
              <CardBody>
