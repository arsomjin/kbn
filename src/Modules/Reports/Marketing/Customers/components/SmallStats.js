...
class SmallStats extends React.Component {
  ...
  render() {
    ...
    const percentageClasses = classNames(
      ...
      typeof increase !== 'undefined' ? `stats-small__percentage--${increase ? 'increase' : 'decrease'}` : ''
    );
    ...
    return (
      <Card small className={cardClasses}>
        <CardBody className={cardBodyClasses} onClick={this.props.onPress}>
          <div className={innerWrapperClasses}>
            <div className={dataFieldClasses}>
              <span className={labelClasses} style={{ fontSize: '14px', whiteSpace: 'nowrap' }}>
                {label}
              </span>
        ...
        {this.props.onPress && (
          <div style={{ position: 'absolute', top: 0, right: 0 }}>
            <InfoCircleOutlined className="text-muted m-2" />
          </div>
        )}
      </Card>
    );
