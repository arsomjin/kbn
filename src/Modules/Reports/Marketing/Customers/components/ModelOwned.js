...
class ModelOwned extends React.Component {
  ...
  render() {
    ...
    return (
      <Card small className="country-stats" style={{ height: '60vh' }}>
        <CardHeader {...(data.length === 0 && { className: 'border-bottom' })}>
          <h6 className="m-0">{title}</h6>
          <div className="block-handle" />
        </CardHeader>

        <CardBody className="p-1 pb-3" style={{ overflowY: 'scroll' }}>
          {data.length === 0 ? (
                  ...
                  <tr key={idx}>
                    <td>
                      {/* <img
                        ...
                        alt={item.title}
                      /> */}
                      {item.ownedModel && models[item.ownedModel] ? models[item.ownedModel].name : 'ไม่ระบุ'}
                    </td>
                    <td className="text-right">{item.counter}</td>
                    <td className="text-right">{item.visitorsPercentage}</td>
                  </tr>
                ))}
              </tbody>
            </table>
