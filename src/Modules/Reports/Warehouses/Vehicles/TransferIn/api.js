...
export const getVehicleTransferInData = ({ month, branchCode, vehicleType, branches }) =>
  new Promise(async (r, j) => {
    ...
    if (branchCode !== 'all') {
      wheres = wheres.concat([['toDestination', '==', branchCode]]);
    }
    ...
    let dBranches = distinctArr(fArr, ['toDestination'])
      .map(gn => gn.toDestination)
      .filter(l => !!l);
    let bKeys = {};
    dBranches.map(br => {
      bKeys[br] = null;
    ...
    let nData = dArr.map(md => {
      distinctArr(fArr, ['toDestination', 'vehicleType'], ['qty'])
        .filter(l => l.vehicleType === md.vehicleType)
        .map(it => {
          md[it.toDestination] = it.qty;
          return it;
        });
      return md;
    });
