...
import { columns, getDecalData, expandedRowRender } from './api';
...
export default () => {
  ...
  const _getData = useCallback(async pCode => {
    try {
      let arr = await getDecalData(pCode);
      setData(arr);
    } catch (e) {
      showWarn(e);
    }
          ...
          return (
            <>
              <div className="page-header p-3 ">
                <Row>
                  <PageTitle sm="8" title="ทะเบียนคุมลอกลาย" subtitle="คลังสินค้า" className="text-sm-left" />
                </Row>
              </div>
