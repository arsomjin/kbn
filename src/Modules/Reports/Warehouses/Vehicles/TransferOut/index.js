...
import { columns, getVehicleTransferOutData, expandedRowRender } from './api';
...
export default () => {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        setLoading(true);
        let result = await getVehicleTransferOutData({ ...headers, branches });
        setLoading(false);
        setData(result);
      } catch (e) {
        showWarn(e);
  ...
  return (
    <Container fluid className="main-content-container p-3">
          ...
          return (
            <>
              <div className="px-3 bg-white">
                <PageTitle sm="8" title="การโอนย้าย(ออก)" subtitle="รายงาน - คลังสินค้า" className="text-sm-left" />
              </div>
      ...
      <div className="d-flex justify-content-center mt-3">
        <label className="text-primary p-3">{`รายการโอนย้าย ${headers.vehicleType}`}</label>
      </div>
      <EditableCellTable
