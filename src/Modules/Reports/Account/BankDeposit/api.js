    ...
    switch (cl.title) {
      ...
      case 'โอนเข้าบัญชี':
        cl.children.map(cd => {
          arr.push({
            title:
              cd.dataIndex === 'transferBy'
                ? 'ชื่อ - นามสกุล'
                : cd.dataIndex === 'aso_acc'
                  ? 'อาซ๊อ'
                  : cd.dataIndex === 'company_acc'
                    ? 'บริษัท'
                    : cd.dataIndex === 'k_benz_acc'
                      ? 'คุณเบนซ์'
                      : cd.title,
    ...
    try {
      ...
      dataArr
        ...
        .map(it => {
          ...
          // }
          payments.map(pm => {
            const { amount, customer, customerName, selfBank, person, personName, paymentType } = pm;
...
export const formatBankDeposit = (dataArr, banks, employees, customers) =>
  new Promise(async (r, j) => {
    if (!Array.isArray(dataArr) || (!!dataArr && dataArr.length === 0)) {
      return r([]);
    }
    try {
      ...
      dataArr
        .filter(l => !l.deleted)
        .map(it => {
          let incomeTitle = 'เงินฝากธนาคารประจำวัน';
          const { depositDate, selfBankId, branchCode, total, depositor } = it;
          let name = banks[selfBankId] ? banks[selfBankId].name : undefined;
          let aso_acc = null;
          let company_acc = null;
          let k_benz_acc = null;
          if (name) {
          ...
          let transferBy = getNameFromEmployeeCode({
            employeeCode: depositor,
            employees
          });
