...
import { columns, getAssessmentData, getColumns, initData } from './api';
...
export default () => {
  ...
  const getData = async branch => {
    try {
      setLoading(true);
      const assessmentData = await getAssessmentData(branch);
      setData(assessmentData);
      assessmentData.yearArr.length > 0 &&
        handleSelect(assessmentData.yearArr[assessmentData.yearArr.length - 1], 'year', assessmentData);
      setLoading(false);
    } catch (e) {
      showWarn(e);
      setLoading(false);
  ...
  return (
    <Container fluid className="main-content-container px-4">
      <PageHeader
        ...
        subtitle="Credit Approval Report"
        hasBranch
        hasAllBranch
      ...
      <div className="d-flex mx-3 mt-3">
        <label className="text-primary text-center">ผลการประเมินรายปี</label>
      </div>
      <EditableCellTable
        ...
        summary={pageData => (
          <TableSummary
            ...
            sumKeys={['approved', 'disapproved', 'waiting', 'total']}
            align="center"
            labelAlign="center"
            sumClassName={['text-success', 'text-danger', 'text-warning', 'text-primary']}
          />
        )}
      ...
      {selected?.year && (
        <div className="d-flex mx-3 mt-3">
          <label className="text-primary text-center">
            {data.monthArr.length > 0 ? `ผลการประเมินรายเดือน ประจำปี ${selected.year}` : 'ผลการประเมินรายเดือน'}
          </label>
        </div>
      )}
      <EditableCellTable
        ...
        <div className="d-flex mx-3 mt-3">
          <label className="text-primary text-center">
            {data.dateArr.length > 0
              ? `ผลการประเมินรายวัน ประจำเดือน ${moment(selected.month, 'YYYY-MM').format('MMMM YYYY')}`
              : 'ผลการประเมินรายวัน'}
          </label>
        </div>
      )}
