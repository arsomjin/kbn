...
export const columns = [
  ...
  {
    title: isMobile ? 'วันที่' : 'วันที่ยกเลิก',
...
export const getColumns = (type, allBranch) => {
  ...
  switch (type) {
    ...
    case 'month':
      result = Object.assign([], columns2, {
        0: {
          title: isMobile ? 'เดือน' : 'เดือนที่ยกเลิก',
          dataIndex: 'month',
          align: 'center',
...
export const formatCancellationData = snap => {
  ...
  let cancelDate = cancel?.date || snap.date;
  // let cancelDate = cancel?.date || null;
  ...
  let doc = {
    ...
    sourceOfData,
    cancelDate,
    cancelMonth: cancelDate ? moment(cancelDate, 'YYYY-MM-DD').format('YYYY-MM') : null,
    cancelYear: cancelDate ? moment(cancelDate, 'YYYY-MM-DD').format('YYYY') : null,
    cancelDetails,
...
export const getCancellationData = branch =>
  new Promise(async (r, j) => {
    try {
      ...
      let arrDate = distinctArr(arr, ['cancelDate'], ['qty']).map((l, i) => {
        const { cancelYear, cancelMonth, cancelDate, branchCode, qty } = l;
        return {
          month: cancelMonth,
          year: cancelYear,
          date: cancelDate,
