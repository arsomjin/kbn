...
export const getChartData = ({ dArr, branch, duration }) => {
  ...
  if (branch !== 'all') {
    dArr = dArr.filter(
      l =>
        l.branchCode === branch &&
        moment(l.date).isSameOrAfter(duration[0]) &&
        moment(l.date).isSameOrBefore(duration[1])
    );
  } else {
    dArr = dArr.filter(l => moment(l.date).isSameOrAfter(duration[0]) && moment(l.date).isSameOrBefore(duration[1]));
  }
  let saleItems = [];
  dArr = dArr.map(l => {
    if (l.items) {
      let mItems = l.items.map(it => ({
        ...
        assessmentResult:
          l?.assessment && typeof l.assessment !== 'undefined' ? (l.assessment.result ? 'ผ่าน' : 'ไม่ผ่าน') : 'N/A'
      }));
    ...
    return {
      ...
      month: moment(l.date, 'YYYY-MM-DD').format('MMM YYYY'),
      assessment: l?.assessment || null,
      assessmentDate: l?.assessment ? l.assessment.date : 'N/A',
      assessmentResult:
        l?.assessment && typeof l.assessment !== 'undefined' ? (l.assessment.result ? 'ผ่าน' : 'ไม่ผ่าน') : 'N/A'
    };
  });
  ...
  vehicleType = sortArr(distinctArr(vehicleType, ['vehicleType'], ['qty']), '-qty').filter(
    l => l?.vehicleType && l.vehicleType
  );
  ...
  return {
    ...
    smallStatsData,
    saleItems
    // sales: distinctArr(saleItems, ['bookNo']).map((l, id) => ({
    //   ...l,
    //   id,
    //   key: id,
    // })),
