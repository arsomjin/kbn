...
import { initData, getCancelledData, getColumns } from './api';
...
export default () => {
  ...
  const getData = async search => {
    try {
      setLoading(true);
      const bookingData = await getCancelledData(search);
      // showLog({ bookingData });
      setData(bookingData);
    ...
    <Container fluid className={`main-content-container ${isMobile ? '' : 'px-3'}`}>
      ...
      >
        <Row className="px-3 pt-3 border-bottom">
          <PageTitle sm="4" title="สรุปยอดยกเลิก" subtitle="งานขาย" className="text-sm-left" />
          <Col md="4">
