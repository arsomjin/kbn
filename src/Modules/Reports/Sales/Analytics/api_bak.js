...
export const getChartData = ({ dArr, branch, duration }) => {
  ...
  let vehicles = saleItems.filter(l => l?.vehicleType && l.vehicleType.startsWith('รถ'));
  showLog({ vehicles, saleItems });
  ...
  // let dateDiff = moment(duration[1], 'YYYY-MM-DD').diff(
  //   moment(duration[0], 'YYYY-MM-DD'),
  //   'days'
  // );
  let monthLabels = getMonths(duration[0], duration[1], 'YYYY-MM-DD', 'MMM YYYY').slice(-12);
  let dateSesstions = sortArr(
    distinctArr(dArr, ['date'], ['qty']).map(l => ({
      date: l.date,
      qty: l.qty
    })),
    'date'
  ...
  let monthSesstions = sortArr(
    distinctArr(dArr, ['month'], ['qty']).map(l => ({
      month: l.month,
      qty: l.qty
    })),
    'month'
