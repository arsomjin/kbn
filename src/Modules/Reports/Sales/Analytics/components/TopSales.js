...
const TopReferrals = ({ title, salesPerson, onPress }) => {
  const { allEmployees } = useSelector(state => state.data);

  return (
    <Card small style={{ height: '100%' }}>
        ...
        {salesPerson.length === 0 ? (
          ...
          <ListGroup small flush className="list-group-small">
            ...
            {salesPerson
              ...
              .slice(0, 10)
              .map((item, idx) => (
                <ListGroupItem key={idx} className="d-flex px-3" onClick={() => onPress(item)}>
                  <span className="text-semibold text-fiord-blue">
                    {allEmployees[item.title]
                      ? `${allEmployees[item.title].firstName}${
                          allEmployees[item.title].nickName ? `(${allEmployees[item.title].nickName})` : ''
                        } ${allEmployees[item.title].lastName}`
                      : 'ไม่ระบุ'}
                  </span>
