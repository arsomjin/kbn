...
import { initData, getSaleData, getColumns } from './api';
...
export default () => {
  ...
  const getData = async search => {
    try {
      setLoading(true);
      const saleData = await getSaleData(search);
      setData(saleData);
      saleData.yearArr.length > 0 && handleSelect(saleData.yearArr[saleData.yearArr.length - 1], 'year', saleData);
      setLoading(false);
    } catch (e) {
      showWarn(e);
      setLoading(false);
  ...
  const handleSelect = (record, type, obj) => {
    ...
    switch (type) {
      ...
      case 'date':
        ...
        history.push('/reports/sale-summary-details', {
          params: {
    ...
    <Container fluid className={`main-content-container ${isMobile ? '' : 'px-3'}`}>
      ...
      >
        <Row className="px-3 pt-3 border-bottom">
          <PageTitle sm="4" title="สรุปงานขาย" subtitle="งานขาย" className="text-sm-left" />
          <Col md="4">
      ...
      <EditableCellTable
        ...
        onRow={(record, rowIndex) => {
          return {
            ...
            onDoubleClick: () => alert('Double clicked me!!')
          };
        }}
        // hasChevron
