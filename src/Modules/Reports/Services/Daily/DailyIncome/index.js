...
import { columns, formatServiceDailyIncome, serviceDailyIncomeSumKeys } from './api';
...
export default () => {
  ...
  const _onValuesChange = async val => {
    try {
      ...
      let check = sData.map(it => ({
        key: it.key,
        cash: it.cash,
        moneyTransfer: it.moneyTransfer,
        total: it.total
      }));
      let fData = await formatServiceDailyIncome(sData);
      showLog({
        check,
        search: { ...searchValues.current, ...val },
        fData,
        sData
      });
      setData(fData);
      ...
      <EditableRowTable
        ...
        summary={pageData => (
          <TableSummary
            ...
            startAt={0}
            sumKeys={serviceDailyIncomeSumKeys}
            align="center"
          />
        )}
