...
const InputItems = ({ items, docId, onChange, grant, readOnly, footer, noItemUpdated, loading }) => {
    ...
    new Promise(async (r, j) => {
      try {
        ...
        if (['unitPrice', 'qty', 'discount'].includes(dataIndex)) {
          mItem.total = (Numb(mItem.unitPrice) * Numb(mItem.qty) - Numb(mItem.discount)).toFixed(2);
        }
