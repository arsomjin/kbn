...
export default () => {
  ...
  const _onConfirm = async () => {
    ...
    try {
      ...
      await arrayForEach(arr, async (it, i) => {
        ...
        await api.setItem({ ...mIt, _key }, 'sections/account/expenseTransfer', _key);
  ...
  const footer = () => (
    <h6 className="m-0">{hasSelected ? `เลือก ${selecteds.length} จาก ${data.length} รายการ` : ''}</h6>
  );
  ...
  return (
    <Container fluid className="main-content-container p-3">
      <PageTitle sm="4" title="บันทึกรอบการโอนเงิน" subtitle="บัญชี" className="text-sm-left" />
      <Form form={form} initialValues={InitValues} layout="vertical" size="small" onValuesChange={_onValuesChange}>
        <Row className="page-header py-3 mx-3 align-items-center">
          ...
          <Col md="6">
            <Form.Item label="รายการ" name="category">
              <TransferCycleItemSelector style={{ display: 'flex' }} hasAll defaultValue="all" />
            </Form.Item>
          </Col>
        </Row>
      </Form>
      ...
      <EditableCellTable
        ...
        size="small"
        summary={pageData => (
          <TableSummary pageData={pageData} dataLength={columns.length} startAt={5} sumKeys={['total']} />
        )}
        loading={loading}
        rowSelection={{
          type: 'checkbox',
