...
const DataTable = ({ expenseCategories, originData, expenseNames, selectedDate, branchCode, isInput }) => {
  ...
  const _onChangeTotal = useCallback(
    val => {
      //  showLog('val', val);
      let newData = [...data];
      let nId = data.findIndex(l => l.expenseCategoryId === val.expenseCategoryId);
      if (nId > -1) {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'total',
      render: text => (
        <h6 className={Number(text) > 0 ? 'text-primary' : 'text-light'}>{numeral(text).format('0,0.00')}</h6>
      )
    }
  ];
    ...
    <Table
      ...
      expandable={{
        expandedRowRender: record => {
          return (
            <ExpandableTable
              originData={originData.filter(l => l.expenseCategoryId === record.expenseCategoryId)}
              record={record}
              expenseNames={expenseNames.filter(l => l.expenseCategoryId === record.expenseCategoryId)}
