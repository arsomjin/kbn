...
export default ({ expenses, expenseNames, selectedDate, branchCode, expenseType }) => {
  ...
  const onUpdate = row => {
    ...
    if (Number(mRow.unitPrice) > Numb(mRow.discount)) {
      mRow.total = (Number(mRow.unitPrice) - Numb(mRow.discount)) * Numb(mRow.import);
    }
  ...
  const onConfirm = useCallback(
    async values => {
      try {
        ...
        mValues.taxInvoiceDate = moment(mValues.taxInvoiceDate).format('YYYY-MM-DD');
        ...
        await api.updateItem({ _key: expenseId, expenseId }, 'sections/account/expenses', expenseId);
        // Update Payment Info
        ...
        showSuccess(
          () => resetToInitial(),
          cState.mReceiveNo ? `บันทึกข้อมูลใบรับสินค้าเลขที่ ${cState.mReceiveNo.value} สำเร็จ` : 'บันทึกข้อมูลสำเร็จ',
          true
        );
      } catch (e) {
        showWarn(e);
  ...
  const preConfirm = useCallback(async () => {
    ...
    showConfirm(() => onConfirm(values), `การบันทึกรายการ ${cState.mReceiveNo ? cState.mReceiveNo?.value : ''}`);
    ...
    <div>
      ...
      >
        ...
        <div className="border-bottom bg-white p-3 text-right">
          <Button type="primary" onClick={preConfirm} icon={<CheckOutlined />} className="mr-2 my-2" size="middle">
            บันทึกข้อมูล
          </Button>
        </div>
      </Form>
