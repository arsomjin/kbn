...
import { Row, Col, InputGroup as SInputGroup, InputGroupAddon, InputGroupText, FormInput } from 'shards-react';
        ...
        <Col md="6">
          <Form.Item name="priceType" rules={getRules(['required'])}>
            <InputGroup
              ...
              inputComponent={props => <PriceTypeSelector style={{ width: '100%' }} {...props} />}
            />
          </Form.Item>
        </Col>
        <Col md="6">
...
export const renderSummary = data => {
  showLog({ data });
  const total = data.filter(l => !l.deleted).reduce((sum, elem) => sum + Numb(elem.netTotal), 0);
    ...
    <div className="d-flex flex-column align-items-end">
      <div style={{ width: 420 }}>
        <SInputGroup>
          ...
          <FormInput disabled className="text-right text-primary" value={numeral(total).format('0,0.00')} />
...
export const columns = [
  ...
  },
  {
    ...
    required: true,
    width: 140,
    render: (text, record) => <div>{numeral(Number(record.beforeVAT) + Numb(record.VAT)).format('0,0.00')}</div>,
    align: 'right'
  },
  {
