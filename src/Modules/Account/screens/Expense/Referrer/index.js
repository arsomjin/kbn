...
import { columns, getCreditDataFromSale, initSearchValue, renderHeader } from './api';
  ...
  const handleSelect = async saleData => {
    try {
      if (!saleData?.referringDetails) {
        return showAlert('ไม่มีข้อมูล', `ใบขายเลขที่ ${saleData.saleNo} ไม่มีรายละเอียดค่าแนะนำ`, 'warning');
      }
      if (
  ...
  return (
    <Container fluid className="main-content-container p-3">
      <Row noGutters className="page-header px-3 bg-light">
        <PageTitle sm="4" title="ตรวจสอบค่าแนะนำ" subtitle="บัญชี" className="text-sm-left" />
      </Row>
            ...
            <>
              ...
              <EditableCellTable
                ...
                rowClassName={(record, index) =>
                  record?.deleted ? 'deleted-row' : record?.accountReferrerChecked ? 'recorded-row' : 'editable-row'
                }
              />
            </>
          );
