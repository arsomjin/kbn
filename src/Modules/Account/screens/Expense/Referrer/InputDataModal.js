...
const InputDataModal = ({ selectedData = {}, saleData, visible, onOk, onCancel, title = 'Title', ...mProps }) => {
  ...
  const isRecorded = selectedData && !!saleData?.creditInfo && saleData.creditInfo?.contractAmtReceivedDate;
  ...
  const onConfirm = useCallback(
    async mValues => {
      try {
        ...
        const { forHQ, bank, bankAcc, bankName, total, whTax, saleCutoffDate, saleId, saleItems, saleNo } =
          referringDetails;
        ...
        const expenseRef = firestore.collection('sections').doc('account').collection('expenses').doc(expenseId);
        ...
        {values => {
          ...
          return (
            <Card small className="mb-4">
              ...
              <CardBody>{referrerCheck({ values, isAccountCheck: true })}</CardBody>
            </Card>
          );
        }}
      </Form>
