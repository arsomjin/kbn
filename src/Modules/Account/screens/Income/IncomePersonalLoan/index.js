...
export default () => {
  ...
  const _getData = useCallback(async sValues => {
    try {
      ...
      let mData = [];
      let data = await getSearchData('sections/account/incomes', {
        branchCode,
        startDate,
        endDate,
        incomeSubCategory,
      ...
      data
        ...
        .forEach((itm, id) => {
          ...
          const cValues = cleanValuesBeforeSave({
            id,
            key: id,
            incomeId,
  ...
  return (
    <Container fluid className="main-content-container p-3">
      <Row noGutters className="page-header px-3 bg-light">
        <PageTitle sm="4" title="รับเงิน - สินเชื่อส่วนบุคคล" subtitle="บัญชี" className="text-sm-left" />
        <Col>
          <Stepper
            className="bg-light"
            steps={CommonSteps}
