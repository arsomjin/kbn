...
import { columns, getSKLAccountDataFromSale, initSearchValue, renderHeader } from './api';
...
export default () => {
  const mSearchValues = { ...initSearchValue, saleType: 'sklLeasing' };
  ...
  const _getData = useCallback(async sValues => {
    try {
      ...
      if (sValues?.saleNo && sValues.saleNo !== 'all') {
        mData = await getSearchData('sections/sales/vehicles', { saleNo: sValues.saleNo, saleType: sValues.saleType }, [
          'saleNo',
          'date'
        ]);
  ...
  const handleSelect = async sale => {
    try {
      ...
      if (!sale?.creditRecorded) {
        return showWarning(`ฝ่ายสินเชื่อยังไม่ได้บันทึกข้อมูล เลขที่ ${sale?.saleNo || ''}`);
      }
      if (sale?.creditInfo) {
        saleData = {
          ...saleData,
      ...
      let accountData = await getSKLAccountDataFromSale(saleData, {});
  ...
  return (
    <Container fluid className="main-content-container p-3">
      <Row noGutters className="page-header px-3 bg-light">
        <PageTitle sm="4" title="รับเงิน - SKL" subtitle="บัญชี" className="text-sm-left" />
        <Col>
          <Stepper
            className="bg-light"
            steps={CommonSteps}
