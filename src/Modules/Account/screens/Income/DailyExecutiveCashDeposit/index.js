...
import { getColumns, getInitItem, renderInput } from './helper';
...
export default () => {
  ...
  useEffect(() => {
    ...
    const readOnly = onBack?.path ? onBack.path === '/reports/daily-executive-cash-deposit' : false;
  ...
  useEffect(() => {
    ...
    const query = firestore
      .collection('sections')
      .doc('account')
      .collection('executiveCashDeposit')
  ...
  const onUpdate = async row => {
    try {
      ...
      await api.updateItem(newValues, 'sections/account/executiveCashDeposit', newValues._key);
      load(false);
    } catch (e) {
      showWarn(e);
      load(false);
  ...
  const onDelete = async key => {
    try {
      ...
      delete nValues.id;
      let newValues = checkEditRecord(nValues, data, user);
      await api.updateItem(newValues, 'sections/account/executiveCashDeposit', nData[index]._key);
      load(false);
    } catch (e) {
      showWarn(e);
      load(false);
  ...
  const _onConfirmOrder = async values => {
    try {
      ...
      const executiveCashDepositRef = firestore
        .collection('sections')
        .doc('account')
        .collection('executiveCashDeposit')
        .doc(values.depositId);
      // Add executiveCashDeposit order.
      const docSnap = await executiveCashDepositRef.get();
      if (docSnap.exists) {
        executiveCashDepositRef.update(mValues);
      } else {
        executiveCashDepositRef.set(mValues);
      }
      // Record log.
      api.addLog(
        mProps.isEdit
      ...
      errorHandler({
        code: e?.code || '',
        message: e?.message || '',
        snap: { ...values, module: 'DailyExecutiveCashDeposit' }
      });
      load(false);
    }
  };
