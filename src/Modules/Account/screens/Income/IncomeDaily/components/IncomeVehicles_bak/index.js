...
import { firstKey, showWarn, showConfirm, load, cleanValuesBeforeSave, showMessageBar } from 'functions';
  ...
  const _onValuesChange = async val => {
    try {
      if (sKeys.includes(firstKey(val))) {
        let mData = [];
        let sValues = { ...searchValuesRef.current, ...val };
        if (firstKey(val) === 'saleNo') {
          mData = await getSalesData(sValues, true);
          setSale({
            doc: {
              ...mData[0],
              title: `ใบขายเลขที่ ${mData[0]?.saleNo}`
            },
            type: 'sale'
          });
          form2.setFieldsValue({
            total: _getNetIncomeFromValues(mData[0]),
            ...(mData[0].saleType === 'baac' && {
              amtBaacDebtor: mData[0].amtBaacDebtor
            })
          });
          form.setFieldsValue({
            bookNo: null
          ...
          mData = await getBookingData(sValues);
          setSale({
            doc: {
              ...mData[0],
              title: `ใบจองเลขที่ ${mData[0]?.bookNo}`
            },
            type: 'booking'
          });
          form2.setFieldsValue({
            incomeType: 'reservation',
            total: mData[0].amtReceived
          });
          form.setFieldsValue({
            saleNo: null
          });
    ...
    try {
      ...
      if (['licensePlateFee', 'installment', 'kbnLeasing', 'other'].includes(docType)) {
        ...
        mValues = {
          ...
          accBranchCode: branchCode,
          accDate: date,
          branchCode: saleDoc.doc.branchCode,
          date: saleDoc.doc.date,
          saleNo,
          saleId,
          bookNo,
          bookId,
      ...
      showConfirm(
        ...
        `บันทึกข้อมูลรับเงินประจำวัน ${
          ['licensePlateFee', 'installment', 'other'].includes(docType)
            ? IncomeType[docType]
            : saleDoc.type === 'sale'
      ...
      {!['licensePlateFee', 'installment', 'kbnLeasing', 'other'].includes(docType) && (
        <Collapse className="my-3" activeKey={saleDoc.doc?.saleNo || saleDoc.doc?.bookNo ? '1' : undefined}>
          <Collapse.Panel header={saleDoc.doc?.title || 'ข้อมูลจากฝ่ายขาย'} key="1">
            {saleDoc.type === 'sale' ? (
              <SaleViewer sale={saleDoc.doc} grant={grant} readOnly noReferrerDetails noGuarantor isAccount />
            ) : (
              <BookViewer sale={saleDoc.doc} grant={grant} readOnly noReferrerDetails isAccount />
            )}
          </Collapse.Panel>
        </Collapse>
      )}
                ...
                <Row>
                  <Col md="4">
                    <Form.Item
                      ...
                      rules={[
                        {
                          required: !['kbnLeasing', 'other'].includes(values.incomeType),
