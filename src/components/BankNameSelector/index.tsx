...
import { showWarn, showSuccess2 } from 'utils/functions';
...
const BankNameSelector = forwardRef<any, BankNameSelectorProps>(
  ({ onChange, placeholder, ...props }, ref) => {
    ...
    const handleOk = async (values: { name: string; remark?: string | null }) => {
      if (!user) {
        showWarn('User not authenticated');
        return;
      }
      ...
      try {
        ...
        setLoading(false);
        showSuccess2(() => setShowAddNew(false), 'บันทึกข้อมูลสำเร็จ');
        setShowAddNew(false);
      } catch (e) {
