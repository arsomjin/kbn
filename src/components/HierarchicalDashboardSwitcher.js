...
import { Select, Card, Row, Col, Button, Typography, Badge, Divider } from 'antd';
import { 
  BankOutlined, 
  ShopOutlined, 
  UserOutlined, 
...
const HierarchicalDashboardSwitcher = ({ 
    ...
    if (selectedProvince) {
      ...
      if (isExecutive || isSuperAdmin) {
        ...
        if (accessibleBranches && accessibleBranches.length > 0) {
          provinceBranches = accessibleBranches.filter(branch => 
          ...
          const selectedProvinceData = allProvinces.find(p => 
            (p.key || p.id) === selectedProvince
          );
        ...
        provinceBranches = accessibleBranches.filter(branch => 
          branch.provinceId === selectedProvince || branch.provinceName === selectedProvince
        );
      }
  ...
  const shouldShowSwitcher = () => {
    ...
    const isStaffOnly = !isSuperAdmin && !isExecutive && !hasProvinceAccess && !hasBranchAccessOnly;
    return !isStaffOnly;
  };
  ...
  const getAvailableRoles = () => {
    ...
    if (isExecutive || isSuperAdmin) {
      ...
      roles.push(
        { key: 'EXECUTIVE', name: 'ระดับผู้บริหาร', icon: <GlobalOutlined />, level: 1 },
        { key: 'PROVINCE_MANAGER', name: 'ผู้จัดการระดับจังหวัด', icon: <BankOutlined />, level: 2 },
        { key: 'BRANCH_MANAGER', name: 'ผู้จัดการสาขา', icon: <ShopOutlined />, level: 3 },
  ...
  const currentRoleName = availableRoles.find(r => r.key === currentUserRole)?.name || 'ไม่ทราบ';
  const viewingRoleName = availableRoles.find(r => r.key === currentViewingRole)?.name || currentRoleName;
  ...
  const renderExpandedContent = () => (
    ...
    >
      ...
      <Row gutter={[16, 16]} align="middle">
        ...
        <Col xs={24} sm={8} md={6}>
          <div>
            <Text strong style={{ fontSize: '11px', color: '#666' }}>บทบาทปัจจุบัน:</Text>
            <div style={{ marginTop: '4px' }}>
              <Badge 
                color="#52c41a" 
                text={<Text style={{ fontSize: '12px', fontWeight: 'bold' }}>{currentRoleName}</Text>}
              />
            </div>
          </div>
        </Col>
        ...
        <Col xs={24} sm={8} md={6}>
          <div>
            <Text strong style={{ fontSize: '11px', color: '#666' }}>ดูในมุมมอง:</Text>
            <Select
              ...
              {availableRoles.map(role => (
                <Option key={role.key} value={role.key} label={role.name}>
                  <div style={{ display: 'flex', alignItems: 'center' }}>
                    {role.icon}
                    <span style={{ marginLeft: '8px', fontSize: '12px' }}>{role.name}</span>
                  </div>
                </Option>
              ))}
            </Select>
        ...
        {(isExecutive || isSuperAdmin || userProvinces?.length > 1) && (
          <Col xs={24} sm={8} md={6}>
            <div>
              <Text strong style={{ fontSize: '11px', color: '#666' }}>จังหวัด:</Text>
              <Select
              ...
              >
                {getProvincesToDisplay().map(province => {
                  return (
                    <Option key={province.key || province.id} value={province.key || province.id}>
        ...
        {(currentViewingRole === 'BRANCH_MANAGER' || currentViewingRole === 'STAFF') && availableBranches.length > 0 && (
          <Col xs={24} sm={8} md={6}>
            <div>
              <Text strong style={{ fontSize: '11px', color: '#666' }}>สาขา:</Text>
              <Select
              ...
              >
                {availableBranches.map(branch => (
                  <Option key={branch.branchCode || branch.id} value={branch.branchCode || branch.id}>
