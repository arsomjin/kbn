...
import { Form, Input, Select, DatePicker, InputNumber, Checkbox, Radio, Switch, Button, Row, Col, Space } from 'antd';
...
const AntdFormWrapper = forwardRef(({
  ...
  const { hasPermission, accessibleProvinces, accessibleBranches } = usePermissions();
  ...
  const renderField = (field) => {
    ...
    const formItemProps = {
      ...
      name,
      label,
      rules: required ? [{ required: true, message: `กรุณาป้อน${label}` }, ...rules] : rules,
      tooltip,
      extra,
      dependencies,
      hidden
    ...
    switch (type) {
      ...
      case 'custom':
        return (
          <Form.Item {...formItemProps}>
            {field.render && field.render({ form, disabled: disabled || fieldDisabled, ...fieldProps })}
          </Form.Item>
        );
