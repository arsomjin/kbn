...
import { formatNumber, parser } from 'functions';
...
const ArrayInput = ({ name, columns, addText, disabled, readOnly, form, hasSum, ...props }) => {
  ...
  let gTotal = 0;
  if (hasToSum) {
    gTotal = arr.reduce((sum, elem) => sum + Numb(parser(elem?.unitPrice || 0)) * Numb(parser(elem?.qty || 0)), 0);
  } else {
              ...
              return (
                <AInput.Group key={field.key} compact className="d-flex flex-row">
                  {columns &&
                    columns.map((fld, i) => {
                      const isNumber =
                        (fld.number || fld.decimals || fld.currency) && !(hasToSum && fld.name === 'total');
                      ...
                      return (
                        ...
                        >
                          <Input
                            placeholder={fld.placeholder || (fld.number ? 'ป้อนจำนวน' : 'ป้อนข้อมูล')}
                            onKeyDown={e => {
                            ...
                            onBlur={() => {
                              if (form && arr[field.name]) {
                                ...
                                let total = !!qty && !!unitPrice ? Numb(parser(qty)) * Numb(parser(unitPrice)) : null;
                                if (!!total) {
