...
export const getRenderColumns = <T extends Record<string, any>>(
  ...
  return columns.map((col, colIndex) => {
    let formattedTitle: React.ReactNode = typeof col.title === 'function' ? undefined : <div className="text-center">{col.title}</div>;
    if (col?.titleAlign) {
    ...
    let mCol: ColumnType<T> = {
      ...
      align: col.align === 'start' ? 'left' : col.align === 'end' ? 'right' :
        (col.align === 'left' || col.align === 'right' || col.align === 'center') ? col.align : 'left'
    };
      ...
      switch (col.dataIndex) {
        ...
        case "hasWHTax":
          mCol = {
            ...
            render: (text: string) => (
              <div className={!text && text !== "0" ? "transparent" : ""}>
                {text && (WitholdingTax as Record<string, string>)[text] ? (WitholdingTax as Record<string, string>)[text] : text || "-"}
              </div>
            )
          };
          break;
        case "whTaxDoc":
          mCol = {
            ...
            render: (text: string) => (
              <div className={!text && text !== "0" ? "transparent" : ""}>
                {text && (WitholdingTaxDoc as Record<string, string>)[text] ? (WitholdingTaxDoc as Record<string, string>)[text] : text || "-"}
              </div>
            )
          };
          break;
        case "paymentType":
          mCol = {
            ...
            render: (text: string) => (
              <div className={!text && text !== "0" ? "transparent" : ""}>
                {text && (PaymentType as Record<string, string>)[text] ? (PaymentType as Record<string, string>)[text] : text || "-"}
              </div>
            )
          };
          break;
        case "paymentMethod":
          mCol = {
            ...
            render: (text: string) => (
              <div className={!text && text !== "0" ? "transparent" : ""}>
                {text && (PaymentMethod as Record<string, string>)[text] ? (PaymentMethod as Record<string, string>)[text] : text || "-"}
              </div>
            )
          };
          break;
        case "buyType":
          mCol = {
            ...
            render: (text: string) => (
              <div className={!text && text !== "0" ? "transparent" : ""}>
                {text && (BuyType as Record<string, string>)[text] ? (BuyType as Record<string, string>)[text] : text || "-"}
              </div>
            )
          };
          break;
        case "vehicleItemType":
          mCol = {
            ...mCol,
            render: (text: string) => (
              <div className={!text && text !== "0" ? "transparent" : ""}>{text ? (VehicleItemType as Record<string, string>)[text] ?? text : "-"}</div>
            )
          };
          break;
        case "importType":
