...
import { getStartDateFromDuration } from 'functions';
const { Option } = Select;

const initDuration = [moment().startOf('week').format('YYYY-MM-DD'), moment().format('YYYY-MM-DD')];

export default forwardRef(({ placeholder, onChange, defaultDuration, value, hasAll, ...props }, ref) => {
  const mDuration = defaultDuration ? [getStartDateFromDuration(defaultDuration), initDuration[1]] : initDuration;
  ...
  return (
    <div>
      <Select ref={selectRef} placeholder={placeholder || 'ช่วงเวลา'} value={val} onChange={_onChange} {...props}>
