...
export type ModalContextType = {
  showConfirm: (options: { title?: string; content: React.ReactNode; onOk?: () => void; onCancel?: () => void }) => void;
  showSuccess: (content: string, duration?: number, onClose?: () => void) => void;
  showWarning: (content: string, duration?: number, onClose?: () => void) => void;
  showActionSheet: (options: { title?: string; content: React.ReactNode; onOk?: () => void; onCancel?: () => void }) => void;
};
...
export const ModalProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const contextValue = useMemo<ModalContextType>(() => ({
    showConfirm: ({ title = "ยืนยัน", content, onOk, onCancel }) => {
      modal.confirm({
        title,
        content,
        onOk,
    ...
    showActionSheet: ({ title = "ยืนยัน", content, onOk, onCancel }) => {
      modal.confirm({
        title,
        content,
        onOk,
