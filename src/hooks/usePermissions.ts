...
export const usePermissions = () => {
  ...
  const userProvinces = useMemo<Province[]>(() => {
    ...
    // Admin roles and General Manager can access all provinces
    if (isInRoleCategory(user.role as RoleType, RoleCategory.GENERAL_MANAGER)) {
      return allProvinces;
    }

    // Province Managers with multiple province access
    if (user.role === ROLES.PROVINCE_MANAGER && user.accessibleProvinceIds) {
  ...
  const hasProvinceAccess = useCallback(
    (provinceId: string): boolean => {
      ...
      // Admin roles and General Managers have access to all provinces
      if (isInRoleCategory(user.role as RoleType, RoleCategory.GENERAL_MANAGER)) {
        return true;
      }
