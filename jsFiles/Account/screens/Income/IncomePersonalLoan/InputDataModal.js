  ...
  const onConfirm = useCallback(
    async (mValues) => {
      try {
        ...
        const loanIncomeRef = firestore
          .collection('sections')
          .doc('account')
          .collection('incomePersonalLoan');
        // Add account order.
        await loanIncomeRef.doc(newValues.loanIncomeId).set(newValues);

        let income = await getDoc(
          'sections',
          `account/incomes/${newValues.incomeId}`
        );
        let pLoanPayments = []
        const { loanIncomeId, incomeId, payDate, created, createdBy, amtReceived, loanIncomeReceiver, pLoanNo, selfBankId } = newValues
        let incomeItem = { loanIncomeId, incomeId, payDate, created, createdBy, amtReceived, loanIncomeReceiver, pLoanNo, selfBankId }
  ...
  const onPreConfirm = useCallback(
    async (values) => {
      try {
        ...
        showLog({
          values, vValues, mValues, preConfirm: {
            ...record,
            ...mValues,
          }
        });
