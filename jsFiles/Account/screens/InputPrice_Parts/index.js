...
export default () => {
      ...
      if (changeKey === 'billNoSKC') {
        ...
        if (snap) {
          ...
          if (warehouseCheckedArr.length === 0) {
            showAlert(
              'ยังไม่ได้ตรวจรับ',
              `ฝ่ายคลังสินค้า ยังไม่ได้ตรวจรับ ${
                warehouseNotCheckedArr[0]?.billNoSKC || ''
              }`
            );
            return;
          }
          ...
          if (warehouseCheckedArr.length > 0) {
            ...
            await arrayForEach(
              ...
              async (it, id) => {
                ...
                let lpSnap = await checkCollection('data/products/partList', [
                  ['pCode', '==', productPCode],
                ]);
                let lp = null;
                if (lpSnap) {
  ...
  const onPriceTypeChange = (priceType) => {
    ...
    let arr = items.map((it) => {
      let unitPrice =
        priceType === 'separateVat'
          ? (Number(it.unitPrice_original) / 1.07).toFixed(3)
    ...
    let total = arr.reduce(
      (sum, elem) =>
        sum +
        (Number(elem?.qty) * Numb(elem?.unitPrice) - Numb(elem?.discount)),
      0
    );
    // showLog({ total });
