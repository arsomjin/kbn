...
export const getCreditDataFromSale = (sale) =>
  new Promise(async (r, j) => {
    try {
      ...
      let refWHTax = sale?.amtReferrer
        ? Numb(parser(sale.amtReferrer)) * 0.05
        : null;
      let refTotal = sale?.amtReferrer
        ? Numb(parser(sale.amtReferrer)) - refWHTax
        : null;
      const hasReferrer = !!sale.referrer?.firstName;
      let itemType = !!sale?.items
        ? uniq(sale.items.map((it) => it.vehicleType)).filter((l) => !!l)
        : [];
      if (sale?.creditInfo) {
        result = {
          ...initCreditValues,
        ...
        let initSnap = {
          ...
          hasReferrer,
          referringDetails,
          sendTransferDate: referringDetails?.forHQ
            ? referringDetails.forHQ?.sendTransferDate || null
            : null,
          actualTransferDate: referringDetails?.forHQ
            ? referringDetails.forHQ?.actualTransferDate || null
            : null,
          itemType,
        };
