...
import ExpenseHQ from './components';
...
export default ({
  ...
  const { users, branches, dealers, banks } = useSelector(
    (state) => state.data
  );
  ...
  // New state to track the header form's date
  ...
  const [branchCode, setBranch] = useState(
    order?.branchCode || user.branch || '0450'
  );
  ...
  useEffect(() => {
    ...
    if (isEdit) {
      ...
      // Also update headerDate state if editing
      setHeaderDate(date);
    } else {
      _resetInitState();
    }
  ...
  const _onValuesChange = (val) => {
    ...
    if (firstKey(val) === 'branchCode') {
      ...
      // Update the headerDate state so we know the header's date
      setHeaderDate(val.date);
  ...
  const _onValuesChange2 = (val) => {
    if (firstKey(val) === 'receiver') {
      form2.setFieldsValue({
        bank: dealers[val.receiver].dealerBank || null,
        accNo: dealers[val.receiver].dealerBankAccNo || null,
        bankName: dealers[val.receiver].dealerBankName || null,
      });
    }
    if (['total', 'priceType', 'hasWHTax'].includes(firstKey(val))) {
      ...
      form2.setFieldsValue({
        ...
        netTotal:
          Numb(values.total) +
          (values.priceType === 'separateVat' ? Numb(VAT) : 0) -
          Numb(whTax),
      });
    }
  };
  ...
  const _onPreConfirm = async () => {
    try {
      ...
      if (nItems.length === 0) {
        return showAlert('ตรวจสอบข้อมูล', 'ยังไม่มีรายการที่บันทึก', 'warning');
      }
      let mValues = { ...values };
      const total = data.reduce((sum, elem) => sum + Numb(elem?.total || 0), 0);
      const beforeVAT = data.reduce(
        (sum, elem) => sum + Numb(elem?.beforeVAT || 0),
        0
      );
      ...
      const netTotal = data.reduce(
        (sum, elem) => sum + Numb(elem?.netTotal || 0),
        0
      );
      ...
      if (!nProps.isEdit) {
        ...
        if (
          ...
          !!eDoc?.created &&
          !!eDoc?.date &&
          eDoc?.expenseId !== mValues.expenseId
        ) {
      ...
      showConfirm(
        ...
        `การบันทึกรายจ่าย${ExpenseType[expenseType]} สาขา${branches[mValues.branchCode].branchName
        } วันที่ ${moment(mValues.date, 'YYYY-MM-DD').format(
          'DD/MM/YYYY'
        )} จำนวน ${nItems.length} รายการ`
      );
    } catch (e) {
      showWarn(e);
    }
  ...
  const handleLedgerRecordsUpdate = async (mValues) => {
    ...
    const ledgerTotal = mValues.reduce(
      (sum, elem) => sum + Numb(elem.netTotal),
      0
    );
    let nData = [...data];
    ...
    if (eIndex > -1) {
      ...
      nData[eIndex].ledgerCompleted =
        ledgerTotal === Numb(nData[eIndex].netTotal);
  ...
  const handleInvoiceUpdate = async (newData) => {
    ...
    try {
      ...
      showSuccess(
        () => showLog('Success'),
        `บันทึกข้อมูลใบกำกับภาษีเลขที่ ${newData.taxInvoiceNo} สำเร็จ`
      );
    } catch (e) {
      showWarn(e);
    }
  ...
  const handleBankInfoUpdate = async (newData) => {
    const expenseKey = expenseModal.expense.expenseItemId;
    try {
      let nData = [...data];
      let eIndex = nData.findIndex((l) => l._key === expenseKey);
      if (eIndex > -1) {
        nData[eIndex].bank = newData.bank;
        nData[eIndex].accNo = newData.accNo;
        nData[eIndex].bankName = newData.bankName;
        setData(nData);
        //  showLog({ nData });
      }
      showSuccess(() => showLog('Success'));
    } catch (e) {
      showWarn(e);
    }
  };
  ...
  return (
    <div className="bg-white px-3 py-3">
      ...
      {/* Lower Form: disabled if headerDate is not set */}
      <fieldset disabled={!headerDate} style={{ border: 'none', padding: 0 }}>
        {!headerDate && (
          <div style={{ marginBottom: 16, color: 'red' }}>
            กรุณากรอกวันที่ในส่วนหัว เพื่อเปิดใช้งานแบบฟอร์มด้านล่าง
          </div>
        )}
        <Form
          form={form2}
              ...
              <>
                <Collapse className="mb-3">
                  <Collapse.Panel header="บันทึกข้อมูล" key="1">
                    {renderInput({ values, user, grant, readOnly })}
      ...
      <Modal
        title={`เลขที่: ${expenseModal.expense.docNo} | ผู้จำหน่าย ${dealers[expenseModal.expense.receiver]
            ? dealers[expenseModal.expense.receiver].dealerName
            : ''
          } | จำนวนเงินโอน ${numeral(expenseModal.expense.netTotal).format(
            '0,0.00'
          )} บาท`}
        ...
        footer={[
          <Button
            key="close"
            onClick={() => setExpenseModal({ visible: false, expense: {} })}
          >
            ปิด
          </Button>,
        ]}
