        ...
        <Col md="6">
          <Form.Item name="department" rules={getRules(['required'])}>
            <InputGroup
              ...
              addonBefore="แผนก"
              inputComponent={(props) => (
                <DepartmentSelector disabled={!grant || readOnly} {...props} />
              )}
            />
          </Form.Item>
        </Col>
...
export const renderSummary = (data, form, branches, payByOther) => {
  ...
  const totalOther = fPayByOther.reduce(
  ...
  return (
    <Row>
      <Col>
        <PageSummary
          title={`สรุปรายการรับ${ExpenseType['dailyChange']} รายการจ่ายเงิน`}
          data={sumData}
        />
      </Col>
      <Col>
...
export const handleUpdate = (filters, nProps, user) =>
  new Promise(async (r, j) => {
    ...
    try {
      ...
      const totalDeposit = changeDeposit
        .filter((l) => !l.deleted)
        .reduce((sum, elem) => sum + Numb(elem.total), 0);
