...
const ExpenseForm: React.FC<ExpenseFormProps> = ({
  ...
  const [headerDate, setHeaderDate] = useState(
    headerForm.getFieldValue("date") || null
  );
    ...
    (initValue = {}) => {
      ...
      if (!deepEqual(curValues, newValues)) {
        ...
        itemForm.setFieldsValue(
          getInitItem(null, initValue.branchCode || branchCode)
        );
        setData([]);
      }
    },
  ...
  const updateData = useCallback(
    async (filters) => {
      try {
        ...
        setLoading(true);
        const updates = await handleUpdate(
          filters,
          { order, readOnly, onBack, isEdit },
          user
        );
        if (!updates) {
          setLoading(false);
  ...
  const updateDataArr = (dArr) => {
    ...
    const totalChange = changeDeposit
      .filter((l) => !l.deleted)
  ...
  const onPreConfirm = async (values) => {
    try {
      ...
      showConfirm(
        ...
        `การบันทึกรายจ่ายเงินทอนประจำวัน สาขา${
          branches[mValues.branchCode].branchName
    ...
    <div className="bg-white px-3 py-3">
      ...
      >
        {(values) => {
          ...
          const editData = values.editedBy
            ? getEditArr(values.editedBy, users)
            : [];
          return (
            <>
              ...
              <div className="px-3 bg-white border mb-3 pt-3">
                <Row>
                  <Col md="4">
                    <Form.Item
                      label={`รับ${expenseType === "executive" ? "เงิน" : ""}${
                        ExpenseType[expenseType]
                      }`}
                    >
                      <Row className="justify-content-center">
                        <h6 className="text-primary mx-3">
                          {numeral(changeDepositTotal).format("0,0.00")}
                        </h6>
