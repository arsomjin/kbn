      ...
      api.addItem(
        {
          ...
          branchCode,
          expenseCategoryId: expenseNames[0].expenseCategoryId,
          expenseItemId: expenseNames.find(
            (l) => l.expenseName === values.addExpenseItem
          ).expenseItemId,
    ...
    : [
        {
          ...
          editable: false,
          render: (text) => (
            <a>{moment(text, 'YYYY-MM-DD').format('DD/MM/YY')}</a>
          ),
        },
        {
          title: 'ผู้เบิกจ่าย',
          ...
          editable: false,
          render: (text) => (
            <a>{`${employees[text].prefix} ${employees[text].firstName}`}</a>
          ),
        },
        {
          title: 'รายการ',
    ...
    <>
      ...
      {isInput && (
        ...
        >
          ...
          >
            <Select
              placeholder="เลือกรายการที่ต้องการบันทึกข้อมูล"
              onChange={(ev) => showLog('select', ev)}
              allowClear
            >
              {expenseNames.map((it) =>
                it.expenseName ? (
