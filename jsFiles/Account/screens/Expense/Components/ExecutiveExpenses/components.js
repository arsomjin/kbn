...
const Tab1 = ({ record, branchCode, onUpdate }) => {
  ...
  const [mBank] = useMergeState({
    ...
    bankName:
      bankName ||
      (dealers[record.receiver] ? dealers[record.receiver].dealerName : null),
  });
      ...
      <Col md="6">
        <Form.Item name="expenseBranch">
          <InputGroup
            spans={[10, 14]}
            addonBefore="สาขา"
            inputComponent={(props) => <BranchSelector {...props} />}
          />
        </Form.Item>
      </Col>
    </Row>
        ...
        <Form.Item name="department">
          <InputGroup
            ...
            inputComponent={(props) => (
              <DepartmentSelector style={{ width: '100%' }} {...props} />
            )}
          />
        </Form.Item>
      </Col>
      <Col md="6">
        <Form.Item name="expenseCategoryId" rules={getRules(['required'])}>
          <InputGroup
            ...
            inputComponent={(props) => (
              <ExpenseCategorySelector placement="bottomRight" {...props} />
            )}
            primary
          />
        </Form.Item>
      ...
      <Table.Summary.Row className="bg-light">
        ...
        <Table.Summary.Cell>
          <div style={{ textAlign: 'right' }}>
            <span>
              <Text className="text-primary">
                {`${numeral(totalWHTax).format('0,0.00')}   `}
              </Text>
    ...
    <div>
      <Form
        ...
        initialValues={{
          ...
          netTotal: record.netTotal
            ? numeral(record.netTotal).format('0.00')
            : null,
        }}
        layout="vertical"
        size="small"
      ...
      <EditableCellTable
        ...
        summary={(pageData) => (
          <TableSummary
            pageData={pageData}
            dataLength={ledgerColumns.length}
            startAt={5}
            sumKeys={['netTotal']}
          />
        )}
...
class ExpenseExecutive extends React.Component {
  constructor(props) {
    super(props);
    //  showLog('ExpandedTable props', this.props);
    const { record } = this.props;
    this.state = {
      dataSource: record.ledgerRecords
        ? record.ledgerRecords.map((l, id) => ({ ...l, id, key: id }))
        : [],
      count: 0,
      editingKey: '',
    ...
    return (
      <Tabs type="card" style={{ height: '100%' }}>
        ...
        <TabPane tab="เลขที่บัญชี" key="3">
          <Tab1
            record={this.props.record}
            onUpdate={this.props.handleBankInfoUpdate.bind(this)}
          />
        </TabPane>
      </Tabs>
    );
...
export default ExpenseExecutive;
