...
export default ({
  ...
  const { banks, dealers, users, branches } = useSelector(
    (state) => state.data
  );
  const [form] = Form.useForm();
  const [form2] = Form.useForm();
  // New state to track the date entered in the header form
  ...
  useEffect(() => {
    ...
    if (isEdit) {
      ...
      // If editing, update headerDate state
      setHeaderDate(date);
    } else {
      _resetInitState();
    }
  ...
  const _onValuesChange = (val) => {
    ...
    if (firstKey(val) === 'branchCode') {
      ...
      // When the header date changes, update headerDate state
  ...
  return (
    <div className="bg-white px-3 py-3">
      ...
      {/* Wrap the lower form in a fieldset that is disabled if headerDate is not set */}
      ...
      <Modal
        title={`เลขที่: ${expenseModal.expense.docNo} | ผู้จำหน่าย ${dealers[expenseModal.expense.receiver]
            ...
            : ''
          } | จำนวนเงิน ${numeral(expenseModal.expense.netTotal).format(
            '0,0.00'
          )} บาท`}
