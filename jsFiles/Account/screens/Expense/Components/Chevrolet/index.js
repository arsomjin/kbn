...
export default (props) => {
  ...
  useEffect(() => {
    ...
    const mActive = !mOrder?.date
      ? 0
      : StatusMapToStep[mOrder?.status || 'pending'];
  ...
  const updateDataArr = (dArr) => {
    let result = dArr.map((it, i) => {
      if (it.total) {
        const pTotal =
          Numb(it.total) +
          dArr.slice(0, i).reduce((sum, elem) => sum + Numb(elem?.total), 0);
  ...
  const confirmUpdate = async (mItems) => {
    try {
      ...
      if (!expenseRef.current?.expenseId) {
        ...
        if (isEdit) {
          ...
          if (expenseItemsRef.current && mItems) {
            const itemChanges = getArrayChanges(
          ...
          expenseId =
            expenseRef.current.expenseId || createNewOrderId('KBN-ACC-EXP');
          let values = { ...expenseRef.current, ...expense };
          if (
            expenseRef.current?.created &&
            expense.created - expenseRef.current.created > 300000
          ) {
      ...
      showSuccess(
        () =>
          isEdit
            ? history.push(onBack.path, { params: onBack })
            : showLog('Success!'),
        // : _resetToInitial(),
        'บันทึกข้อมูลสำเร็จ',
        true
  ...
  const _onConfirm = async () => {
    try {
      ...
      showConfirm(
        () => confirmUpdate(nItems),
        `การบันทึกรายจ่าย ${nItems.length} รายการ`
      );
    } catch (e) {
      showWarn(e);
    }
