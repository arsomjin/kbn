...
// Define types for function parameters and props
interface ExpenseFormProps {
  isEdit: boolean;
  expenseType: string;
  onConfirm: (values: any, resetToInitial: () => void) => void;
  order: any;
  readOnly: boolean;
  onBack: any;
  setUnsaved: () => void;
...
const Expense: React.FC<ExpenseProps> = () => {
  ...
  const _changeCategory = (ev: string) => {
    setCategory(ev);
  };

  const _onConfirmOrder = async (values: any, resetToInitial: () => void) => {
    try {
      ...
      if (category === "dailyChange" && !mProps.isEdit) {
        ...
        await firestore.runTransaction(async (transaction) => {
          ...
          mValues.items = (mValues.items || []).map((it: any) => ({
            ...it,
            uniqueKey: [
              it.expenseName,
              it.total,
            ...
            <Col md="4">
              <Form.Item label="ประเภทการจ่ายเงิน">
                <Select
                  ...
                  onChange={(ev: string) => _changeCategory(ev)}
                  value={category}
