...
const ExecutiveDetails: React.FC<ExecutiveDetailsProps> = ({ onOk, onCancel, visible, initDoc }) => {
  ...
  const onPreConfirm = useCallback(
    (values: ExecutiveFormValues) => {
      modal.confirm({
        title: 'ยืนยัน',
        content: `${type === 'add' ? 'สร้าง' : 'บันทึก'}รายชื่อผู้บริหาร ${values.executiveName}`,
        onOk: () => onConfirm(values)
      });
    },
    [onConfirm, type, modal]
  );
  ...
  return (
    ...
    >
      <Form form={executiveForm} layout='horizontal' initialValues={initialValues}>
        ...
        <Form.Item name='executiveCode' rules={[{ required: true, message: 'กรุณาป้อนข้อมูล' }]}>
          {type === 'add' ? (
        ...
        <Form.Item name='executiveName' rules={[{ required: true, message: 'กรุณาป้อนข้อมูล' }]}>
