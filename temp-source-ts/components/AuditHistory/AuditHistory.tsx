...
const AuditHistory: React.FC<AuditHistoryProps> = ({
  ...
  const [localStatusHistory, setLocalStatusHistory] = React.useState<StatusHistoryEntry[]>(statusHistory);
  ...
  const getEmployeeName = (userInfo: any) => {
    ...
    if (userInfo.employeeCode) {
      const emp = Object.values(employees).find((e: any) => e.employeeCode === userInfo.employeeCode);
    ...
    return userInfo.displayName || userInfo.fullName || userInfo.name || t('unknownUser', 'Unknown User');
  };
  ...
  const formatValueWithEmployee = (key: string, value: any) => {
    if (
      (key === 'name' || key === 'createdBy' || key === 'updatedBy' || key === 'employeeCode' || key === 'userInfo') &&
      value &&
      typeof value === 'object' &&
  ...
  const isApproved =
    localStatusHistory.length > 0 && localStatusHistory[localStatusHistory.length - 1].status === 'approved';
