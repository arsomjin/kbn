...
import { useHrMenuChildren } from './useHrMenuChildren';
 ...
 */
const useBasePath = (path: any): string => {
...
const MainLayout: React.FC<MainLayoutProps> = ({ children }) => {
  ...
  const accountBasePath = useBasePath('account');
  const accountMenuChildren = useAccountMenuChildren(accountBasePath);
  const hrBasePath = useBasePath('hr');
  const hrMenuChildren = useHrMenuChildren(hrBasePath);
  const photoURL =
  ...
  const navItemsRaw = [
    ...
    hasPermission(PERMISSIONS.EMPLOYEE_VIEW) &&
      hrMenuChildren.length > 0 && {
        key: 'human-resources',
        icon: <UsergroupAddOutlined />,
        label: t('title') || 'Human Resources',
    ...
    hasPermission(PERMISSIONS.SYSTEM_SETTINGS_VIEW) && {
      ...
      children: [
        ...
        hasRole([ROLES.SUPER_ADMIN, ROLES.EXECUTIVE, ROLES.BRANCH_MANAGER, ROLES.PROVINCE_ADMIN, ROLES.DEVELOPER]) && {
          key: 'branches',
          icon: <BankOutlined />,
