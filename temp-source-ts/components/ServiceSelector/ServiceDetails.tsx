...
const ServiceDetails: React.FC<ServiceDetailsProps> = ({ onOk, onCancel, visible, initDoc }) => {
  ...
  return (
    ...
    >
      <Form form={form} layout='vertical' initialValues={initDoc || initialValues}>
        <Form.Item name='serviceCode' label='รหัสบริการ' rules={[{ required: true, message: 'กรุณากรอกรหัสบริการ' }]}>
          <Input />
        </Form.Item>
        <Form.Item name='serviceName' label='ชื่อบริการ' rules={[{ required: true, message: 'กรุณากรอกชื่อบริการ' }]}>
          <Input />
        </Form.Item>
      </Form>
    </Modal>
