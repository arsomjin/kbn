...
import { ROLES, ROLE_CATEGORIES, RoleCategory, RoleType, UserRole } from '../constants/roles';
...
export const hasAdminAccess = (role?: string): boolean => {
  ...
  return (
    hasRolePrivilege(role as RoleType, ROLES.EXECUTIVE) ||
...
export const getPathFromRole = (userProfile: any, pathName: string): string => {
  const role = userProfile?.role;
  switch (role) {
...
export const getAllowedRolesByCategory = (category: RoleCategory) => mapRoles(ROLE_CATEGORIES[category]);
