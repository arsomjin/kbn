 ...
 */
export function useMergeState<T extends Record<string, unknown>>(initialState: T): [T, StateSetter<T>] {
  const [state, setState] = useState<T>(initialState);

  const setMergedState: StateSetter<T> = newState => {
    setState(prevState => {
      const nextState = typeof newState === 'function' ? (newState as StateUpdater<T>)(prevState) : newState;
